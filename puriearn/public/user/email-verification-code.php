...
$status = sanitize_input($_GET['stats']); // Status code
$vcode = $status - 433456644;

$get_email = sanitize_input($_GET['email']);
...
if (isset($_POST['submit'])) {
    ...
    if ($code != $vcode) {
        ...
        // Verification successful, start the session
        session_start();
        // Store relevant user information in session variables
        $_SESSION['uid'] = $uid;
        $_SESSION['uemail'] = $uemail;

        // Redirect the user to the password reset page or perform any other necessary actions
        ...
        $mailHeaders .= "X-Mailer: PHP/" . phpversion();
            ...
            <html>
              ...
              if (mail($toEmail, $subject, $content, $mailHeaders)) {
                ...
                $msg="Email sent successfully. Please check your email for the password reset code.";
                $type = "success";
                ?>
                <script>
                setTimeout(function () {
                ...
                <div class="d-flex align-items-center justify-content-center vh-100 bg-white">
                    <div class="card rounded-0 border-0 p-5 m-0 w-100">
                        ...
                        <div class="card-header border-0 p-0">
                            ...
                            <p class="text-dark mt-4 mb-5">Please enter the 6-digits code that was sent to your email to proceed.</p>
                        </div>
                        <?php if(isset($msg)) { ?>
                        ...
                        <div class="card-body p-0">
                            <form class="form-horizontal" method="post">
                                <div class="form-group">
                                    <input type="text" class="form-control" name="digits" value="" placeholder="6-digit code"
                                       required>
                                </div>
                
                                <button type="submit" name="submit" class="btn btn-primary w-100 text-uppercase text-white rounded-2 lh-34 ff-heading fw-bold shadow">Verify</button>

                            </form>

                            <form method="post" action="">
                                <input type="hidden" name="email" value="<?php echo $get_email?>">
                                <p class="d-flex align-items-center justify-content-between mt-4 mb-4">
                                  Didn't get code? <a href="signup.php"
                                   class="text-primary fw-bold text-decoration-underline">
                                   <button type="submit" name="submit2" class="text-primary fw-bold text-decoration-underline"
                                    style="background:none;border:none;">Resend</button>
                                  </a></p>
