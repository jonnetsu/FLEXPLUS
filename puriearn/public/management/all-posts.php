...
if(isset($_GET['del']))
      ...
      if(isset($_GET['make']))
		  {
        $sql1="UPDATE `tasks` SET `status`='1' WHERE `id`='".$_GET['id']."' ";
        $result1=mysqli_query($con,$sql1);
		  }
       //Remove user's a vendor status
       if(isset($_GET['remove']))
       {
         $sql1="UPDATE `tasks` SET `status`='0' WHERE `id`='".$_GET['id']."' ";
         $result1=mysqli_query($con,$sql1);
       }
?>
...
<div class="container-fluid py-4">
                  ...
                  <h5 class="card-header">Sponsored Posts</h5>
                      ...
                      <td class="align-middle">
                        ...
                        if($row['status']== '1'){ ?>
                            <a href="?id=<?php echo $row['id'];?>&remove=vendor" 
                               onClick="return confirm('Are you sure you want to remove this post inactive?')" class="btn btn-warning ">
                             Make Inactive
                            </a>
                        <?php }else{ ?>
                          <a href="?id=<?php echo $row['id'];?>&make=vendor" 
                               onClick="return confirm('Are you sure you want to make this post active?')" class="btn btn-primary deactivate-account">
                             Make Active</a>
                        <?php } ?>
                        <a href="edit-post.php?id=<?php echo $row['id'];?>"  class="btn btn-primary ">
                            Edit Post
                            </a>
                        <a href="?id=<?php echo $row['id'];?>&del=delete" 
                        onClick="return confirm('Are you sure you want to delete post?')" class="btn btn-danger deactivate-account">
                        Delete Post</a>
                        </td>
                      </tr>
                    <?php } 
