...
if(isset($_GET['approve'])){
    ...
    $res = mysqli_query($con, $sql);
    if($res){
        $updateQuery = "UPDATE `users` SET `job_balance`=$account_table_name + $pay WHERE `id`='$uid'";
        ...
        if (mysqli_num_rows($result) > 0) {
            while ($row = mysqli_fetch_array($result)) {
                ...
                <tr>
                    ...
                    <td>
                    <?php $query2=mysqli_query($con,"SELECT * FROM `job_advert_screenshots`  WHERE `user_id`='$userId' AND `job_id`='$jobId' ");
                                                while($r=mysqli_fetch_array($query2))
                                                {?>
                                        <img src="../admin/screenshots/<?php echo $r['url'];?>" style="width:100px;">
