...
?>
        <div class="container-fluid py-4">
                  <div class="card" style="padding:30px;">
                    <h5 class="card-header">Approved Tiktok Tasks Review</h5>  
                 <div>
                 <?php if(isset($msg)){ ?>
                ...
                <div class="table-responsive table-wrapper-top text-nowrap" >
                  ...
                  <tbody>
                  <?php 
                  $query="SELECT * FROM `tiktok_user_drafts` WHERE `status`='admin_approval' = 'Approved' ";
                  $result=mysqli_query($con,$query);
                  $cnt=1;
                  if(mysqli_num_rows($result)>0){
                    while($row=mysqli_fetch_array($result)){
                        $url=number_format($row['tiktok_user_drafts']);
                        $userId=$row['user_id'];
                      ?>
                    <tr>
                      <td><?php echo $cnt++;?></td>
